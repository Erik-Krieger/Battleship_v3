...
public class WebSocketServer : NetworkPeer
{
	...
	public WebSocketServer(CancellationToken theCancellationToken = default(CancellationToken))
	{
	...
	public async void Start()
	{
		...
		while (!m_Token.IsCancellationRequested)
		{
			...
			_ = array[0];
			int num = 2;
			ulong num2 = (ulong)array[1] & 0x7FuL;
			switch (num2)
			{
			case 126uL:
			...
			if (num2 != 0L && flag)
			{
				...
				@string = NetworkPeer.DecodeString(array2);
	...
	{
		base.m_NetworkThread = new Thread(Start);
		base.m_NetworkThread.Start();
	}

	private void notifyClientConnected()
	{
		base.PeerConnected = true;
